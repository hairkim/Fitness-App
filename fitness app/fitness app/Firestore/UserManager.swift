...
final class UserManager {
    ...
    func createNewUser(user: DBUser) async throws {
        try userDocument(userId: user.userId).setData(from: user, encoder: Firestore.Encoder())
    }
    ...
    func updateUser(_ user: DBUser) async throws {
        try userDocument(userId: user.userId).setData(from: user, merge: true, encoder: Firestore.Encoder())
    }
    ...
    func addFollower(sender: DBUser, receiver: DBUser) async throws {
        ...
        if !user.followers.contains(sender.userId) {
            ...
            try userRef.setData(from: user, merge: true, encoder: Firestore.Encoder())
